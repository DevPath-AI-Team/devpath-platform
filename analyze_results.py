...
class TestAnalyzer:
    ...
    def load_data(self):
        ...
        try:
            ...
            print("❌ Test sonuçları dosyası bozuk. Frontend'den veri bekleniyor...")
            self.results = {}  # Boş dict - frontend'den veri gelecek
        ...
        try:
            ...
            print("❌ Java curriculum dosyası bozuk!")
            self.curriculum = {"topics": []}
        ...
        try:
            ...
            print("❌ Assessment tests dosyası bozuk!")
            self.tests = {"tests": []}
    ...
    def process_test_answers(self, user_id, name, test_answers):
        ...
        try:
            ...
            for answer in test_answers:
                ...
                if question:
                    ...
                    topic_id = str(question.get('topic_id', 0))
            ...
            # Konu puanlarını hesapla
            final_topic_scores = {}
            for topic_id, scores in topic_scores.items():
                percentage = (scores['correct'] / scores['total']) * 100
                final_topic_scores[topic_id] = round(percentage)

            # Genel puan hesapla
            overall_score = (correct_count / total_questions) * 100
            ...
            test_result = {
                ...
                'time_taken': 0,  # Frontend'den gelebilir
            ...
            # Test sonucunu ekle
            user_data['tests_taken'].append(test_result)

            # Konu puanlarını güncelle
            user_data['topic_scores'] = final_topic_scores

            # Zayıf konuları güncelle (%70 altı)
            weak_topics = [topic_id for topic_id, score in final_topic_scores.items() if score < 70]
            user_data['weak_topics'] = weak_topics
            ...
            print(f"❌ Test cevapları işlenirken hata: {e}")
            return None
    ...
    def get_user_topic_scores(self, user_id):
        """Kullanıcının konu puanlarını al"""
        if user_id in self.results:
            return self.results[user_id].get('topic_scores', {})
        return {}

    def analyze_user(self, user_id):
        """Kullanıcının detaylı analizini yap - Müfredata göre analiz"""
        if user_id not in self.results:
        ...
        # Müfredat bilgilerini analize dahil et
        curriculum_info = {
            "total_topics": len(self.curriculum.get("topics", [])),
            "total_hours": self.curriculum.get("total_hours", 0),
            "language": self.curriculum.get("language", "java")
        }
        ...
        analysis = {
            ...
            "topic_analysis": self.analyze_topics(user_data),  # Müfredata göre analiz
            "test_history": self.analyze_test_history(user_data),
            "recommendations": self.generate_recommendations(user_data),  # Müfredata göre öneriler
    ...
    def analyze_topics(self, user_data):
        """Konu bazlı analiz - Müfredata göre"""
        topic_scores = user_data.get("topic_scores", {})
        weak_topics = user_data.get("weak_topics", [])
        
        topic_analysis = {}
        for topic_id, score in topic_scores.items():
            # Müfredattan konu bilgilerini al
            topic_info = self.get_topic_info(topic_id)
            
            # Müfredat bilgilerini kullanarak analiz yap
            topic_analysis[topic_id] = {
                "name": topic_info.get("name", "Bilinmeyen"),
                "score": score,
                "status": "weak" if topic_id in weak_topics else "strong",
                "level": topic_info.get("level", "unknown"),  # Müfredattan
                "difficulty": topic_info.get("difficulty", "unknown"),  # Müfredattan
                "duration": topic_info.get("duration", 0),  # Müfredattan
                "description": topic_info.get("description", ""),  # Müfredattan
                "prerequisites": topic_info.get("prerequisites", []),  # Müfredattan
                "learning_objectives": topic_info.get("learning_objectives", []),  # Müfredattan
    ...
    def generate_recommendations(self, user_data):
        ...
        # Öncelikli konular için öneriler - Müfredata göre
        for topic_id in weak_topics[:5]:  # İlk 5 zayıf konu
            topic_info = self.get_topic_info(topic_id)  # Müfredattan bilgi al
            score = topic_scores.get(topic_id, 0)
            
            # Müfredattaki prerequisites kontrolü
            prerequisites = topic_info.get("prerequisites", [])
            prerequisites_status = []
            for prereq_id in prerequisites:
                prereq_score = topic_scores.get(str(prereq_id), 0)
                prerequisites_status.append({
                    "topic_id": prereq_id,
                    "topic_name": self.get_topic_info(str(prereq_id)).get("name", "Bilinmeyen"),
                    "score": prereq_score,
                    "ready": prereq_score >= 70
                })
            ...
            recommendation = {
                ...
                "target_score": 70,
                "priority": self.calculate_topic_priority(score, topic_info),
                "estimated_time": topic_info.get("duration", 0),  # Müfredattan
                "level": topic_info.get("level", "unknown"),  # Müfredattan
                "difficulty": topic_info.get("difficulty", "unknown"),  # Müfredattan
                "prerequisites": prerequisites_status,  # Müfredattan ön koşullar
                "learning_objectives": topic_info.get("learning_objectives", []),  # Müfredattan
    ...
    def determine_user_level(self, user_data):
        """Kullanıcının genel seviyesini belirle (3 seviye: beginner, intermediate, advanced)"""
        topic_scores = list(user_data.get("topic_scores", {}).values())
        if not topic_scores:
            return "beginner"
        ...
        # 3 seviye sistemi
        if avg_score < 60:
            return "beginner"      # Başlangıç: 0-60
        elif avg_score < 80:
            return "intermediate"   # Orta: 60-80
        else:
            return "advanced"      # Zor: 80-100
    
    def get_video_recommendations(self, user_id, limit=10, language="tr"):
        """
        Kullanıcının test sonuçlarına göre seviyeye uygun video önerileri getir
        
        Args:
            user_id: Kullanıcı ID'si
            limit: Maksimum video sayısı
            language: Dil tercihi (tr, en, all)
        
        Returns:
            Video önerileri listesi
        """
        if user_id not in self.results:
            return []
        ...
        user_level = self.determine_user_level(user_data)
        ...
        videos = self._get_videos_from_database(
            user_level=user_level,
            weak_topic_ids=weak_topics,
            language=language,
            limit=limit
        )
    ...
    def _get_videos_from_database(self, user_level, weak_topic_ids, language, limit):
        """
        Veritabanından video önerilerini getir
        
        Args:
            user_level: Kullanıcı seviyesi (beginner, intermediate, advanced)
            weak_topic_ids: Zayıf konu ID'leri listesi (string listesi)
            language: Dil tercihi (tr, en, all)
            limit: Maksimum video sayısı
        
        Returns:
            Video önerileri listesi (dict formatında)
        """
        # Video önerileri veritabanından gelecek
        # Bu fonksiyon veritabanı entegrasyonu yapıldığında doldurulacak
        return []
