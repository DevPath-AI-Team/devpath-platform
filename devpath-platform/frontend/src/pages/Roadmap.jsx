...
const Roadmap = () => {
    ...
    useEffect(() => {
        const fetchRoadmap = async () => {
            ...
            try {
                // API'den tüm dillerdeki ilerlemeyi çek
                const response = await axios.get(`http://localhost:8080/api/progress/user/${userId}`);
                const userProgress = response.data;

                if (!userProgress || userProgress.length === 0) {
                     // Kullanıcının seçtiği dile göre dersleri öner
                    const allLessonsResponse = await axios.get(`http://localhost:8080/api/lessons/language/${userLanguage}/level/${userLevel}`);
                ...
                setError("Yol haritası yüklenirken bir hata oluştu. " + (err.response?.data?.message || err.message));
            } finally {
                setIsLoading(false);
            }
        };
