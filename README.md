...
DevPath AI aşağıdaki REST API endpoint'lerini sağlar:

| Method | Endpoint | Açıklama |
|--------|----------|----------|
| `GET` | `/health` | API sağlık kontrolü |
| `GET` | `/api/questions` | Test soruları alma (`?category=all&limit=30`) |
| `POST` | `/api/analyze` | Test sonuçları analizi |
| `POST` | `/api/learning-path/{user_id}` | Kişiselleştirilmiş öğrenme yolu oluşturma |
| `GET` | `/api/user/{user_id}/progress` | Kullanıcı ilerleme raporu |
| `GET` | `/api/user/{user_id}/detailed-report` | Detaylı analiz raporu |
| `GET` | `/api/curriculum` | Java müfredatı alma |

### API Kullanım Örnekleri

#### Test Soruları Alma
```bash
GET /api/questions?category=beginner&limit=10
```
...
```bash
POST /api/learning-path/user_123
Content-Type: application/json

{
  "preferences": {"daily_time": 60},
...
│   ├── generate_test_questions.py # Soru üretimi
